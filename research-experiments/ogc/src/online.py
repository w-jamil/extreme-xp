...
if __name__ == "__main__":
    # Configuration - Use absolute path to ensure data folder is found
    script_dir = os.path.dirname(os.path.abspath(__file__))
    
    # Check if running in Docker (data mounted at /app/data) or locally (../data)
    if os.path.exists('/app/data'):
        DATA_DIRECTORY = '/app/data'
        OUTPUT_CSV_FILE = '/app/results/online_results.csv'
    else:
        DATA_DIRECTORY = os.path.join(os.path.dirname(script_dir), 'data')
        OUTPUT_CSV_FILE = os.path.join(os.path.dirname(script_dir), 'results', 'online_results.csv')
    ...
    print(f"Found {len(all_data_files)} parquet files in data directory")
