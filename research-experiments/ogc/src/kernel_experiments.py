...
def plot_f1_scores(f1_data, dataset_name, output_path=None, highlight='K-GC'):
    """
    ...
    Args:
        ...
        highlight: Algorithm to highlight (default: K-GC)
    """
    plt.figure(figsize=(12, 6))
    
    # Style settings for each algorithm
    styles = {
        'K-GC': {'color': '#e41a1c', 'linewidth': 2.5, 'linestyle': '-', 'marker': 'o', 'markersize': 3},
...
def load_heart_disease(random_split=True, test_ratio=0.2):
    ...
    Returns:
        X_train, y_train, X_test, y_test (scaled, labels in {-1, +1})
    """
    # Load from local parquet file (UCI datasets are in data/kernel/)
    data_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'data', 'kernel')
...
def run_repeated_experiments(n_repeats=10, gamma='tune', max_sv=500, epochs=3, test_ratio=0.3, datasets=None):
    ...
    def get_algorithms(gamma_val, max_sv):
        return {
            ...
            'K-GC': lambda: KernelOGL(gamma=gamma_val, max_sv=max_sv),
    ...
    for dataset_name, split_type in dataset_list:
        ...
        # Load raw data once (UCI datasets are in data/kernel/)
        data_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'data', 'kernel')
        parquet_path = os.path.join(data_dir, f'{dataset_name}.parquet')
    ...
    for dataset_name, _ in dataset_list:
        ...
        for algo in algorithms:
            ...
            linewidth = 3 if algo == 'K-GC' else 1.5
            alpha = 1.0 if algo == 'K-GC' else 0.7
            zorder = 10 if algo == 'K-GC' else 1
...
def run_experiments(gamma='tune', max_sv=1000, epochs=3, subsample=10000, max_train=15000, datasets=None):
    ...
    for idx, (dataset_name, split_type) in enumerate(dataset_list, 1):
        ...
        try:
            ...
            if dataset_name == 'HeartDisease_UCI':
